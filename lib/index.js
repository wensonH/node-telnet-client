...
module.exports = class Telnet extends events.EventEmitter {
  ...
  static getInstance() {
    if (!this.instance) {
      this.instance = new Telnet()
    }
    return this.instance
  }

  connect(opts) {
    if (!this.checkSocket(opts)) return
    let promise
    return promise = new Promise((resolve, reject) => {
      ...
      this.keepaliveInterval = (typeof opts.keepaliveInterval !== 'undefined' ? opts.keepaliveInterval : 12000)
      this.cmd = ''
      ...
      this.socket.on('data', data => {
        // console.log('data %s, %s', data, this.state)
  ...
  exec(cmd, opts, callback) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.write(cmd, () => {
        ...
        // this.once('responsecontinue', continueHandler)
        ...
        if (this.execTimeout) {
          execTimeout = setTimeout(() => {
            ...
            // this.removeListener('responsecontinue', continueHandler)
        ...
        function continueHandler() {
          const data = this.inputBuffer.toString();
          if (execTimeout !== null) {
            clearTimeout(execTimeout)
          }

          if (utils.search(this.inputBuffer, this.cmd) !== -1) {
            this.state = 'standby'
            this.inputBuffer = ''
          }
        }
  ...
  checkSocket(opts) {
    // if socket exist and writable, close
  ...
  _parseData(chunk, callback) {
    ...
    if (this.state === 'getprompt') {
      ...
      this.inputBuffer = this.inputBuffer ? this.inputBuffer + stringData : stringData
      let promptIndex = utils.search(stringData, this.shellPrompt)
      if (utils.search(this.inputBuffer, this.loginPrompt) !== -1) {
      ...
      else if (utils.search(this.inputBuffer, this.passwordPrompt) !== -1 && this.loginPromptReceived) {
        this.state = 'login'
        this.inputBuffer = ''
      ...
      if (promptIndex === -1 && stringData.length !== 0) {
        ...
        return this.emit('responsecontinue')
      }
  ...
  sendKA() {
    if (this.socket.writable) {
      ...
      setInterval(() => {
        // push nul to the socket
        this.socket.write(Buffer.from('0', 'hex'))
